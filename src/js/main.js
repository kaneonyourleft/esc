...
function getDefaultDays(proc, category) {
  const normal = { '탈지': 3, '소성': 3, '환원소성': 3, '평탄화': 5, '도금': 3, '열처리': 2 };
  const bl = { '탈지': 3, '소성': 5, '환원소성': 5, '평탄화': 7, '도금': 3, '열처리': 2 };
  return (category && category.toUpperCase() === 'BL') ? (bl[proc] || 3) : (normal[proc] || 3);
}
